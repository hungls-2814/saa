...
promote crashed mid-flight — prompt repair (REVERT `git checkout HEAD -- docs/features/<fcode>_*` +
...
- `feature` → Spec Layer Check (see `references/research-phase.md`):
  - Spec exists in `docs/features/` AND no changes intended → planner writes `spec:` (the promoted
