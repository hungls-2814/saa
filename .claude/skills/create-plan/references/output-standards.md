...
# spec: docs/features/F###_slug/  # Optional — a PROMOTED, unrevised spec (or written by takumi promote)
...
- **spec**: written by takumi's promote step; the planner sets it directly ONLY for a PROMOTED,
  unrevised spec already in `docs/features/`. **omit the field entirely** otherwise — never guess.
