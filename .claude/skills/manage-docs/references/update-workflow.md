...
**Absent-layer advisory:** that `if/elif` block writes to stderr only and **never** reaches the `doc-writer` prompt — the surgical-edit contract stays untouched. The two layers can't both be missing in a meaningful way: an absent `docs/` mutes the spec-layer advisory, since the layered namespaces (`docs/system/`, `docs/generated/`, …) have nowhere to live without their `docs/` parent. The `≥ 2` threshold is counted **after** test/mock/fixture paths are stripped out. Contract: [`_shared/docs-canonical-mapping.md` § Absent-Layer Advisory](../../_shared/docs-canonical-mapping.md#absent-layer-advisory).
