...
def _check_global_contiguity(
    ...
    # A code that appears in prose ≥2 times is a genuine duplicate.
    prose_text = _prose_only_text(text)
    pat = re.compile(
    ...
    for n, count in prose_counts.items():
        if count > 1:
            # Report the line of the second prose occurrence (cheaply derivable)
            second_line = prose_lines[n][1] if len(prose_lines.get(n, [])) >= 2 else None
            iss = _issue(
