...
| FS.1 | FeatureSpec × F### | W5.6 | yes (fan-out) | **feature-specs pass only** |
...
| FS.5 | Feature spec reviewer (batched 5/reviewer) *(Review zone)* | FS.1 | yes (batches parallel) | feature-specs pass only |
...
```
eff_lang   = normalize_lang(--lang) or state.primary_lang or "en"
...
docs_root  = "docs" if eff_lang == "en" else f"docs/{eff_lang}"
```
...
// dirHasChildren(p) ≝ p is a directory containing ≥1 non-empty file (e.g. docs/features/F001/…).
const passPresent = {
  "core":          () => existsNonEmpty("docs/system/overview.md"),
  "feature-specs": () => dirHasChildren("docs/features"),
  "flows":         () => dirHasChildren("docs/flows"),
  "glossary":      () => existsNonEmpty("docs/system/glossary.md"),
  "api-contracts": () => existsNonEmpty("docs/generated/api-contracts.md"),
  "screen-specs":  () => dirHasChildren("docs/screens"),
}
