...
**SYSTEM fan-out:** spawn one researcher **per confirmed feature** in bounded batches
(`REBUILD_FS_BATCH_SIZE`, default 5: ≤20 features = flat fan-out, >20 = sequential batches — mirrors
rebuild-spec FS.1). Each researcher receives its feature's row from `feature-list.md` (intent +
