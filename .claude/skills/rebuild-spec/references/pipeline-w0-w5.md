...
// Enter the gate prompt if: (a) resume from halt, OR (b) first-time trigger (bootable stack or
// explicit flag) AND no prior gate decision, OR (c) --full forces a fresh prompt.
if (isResumingFromHalt || ((hasBootableStack || explicitProbeRoutes) && (!priorProbeGate || flags.includes("--full")))) {
...
const rl_reran = mode === "full" || affected_waves.includes("Wave1: route-list")
if (rl_reran) {
