...
- `## Screen List` table (3-column: `Screen Name | What User Sees | What User Can Do`, per `templates/screens-template.md`) MANDATORY for UI/mixed features. Heading MUST be `## Screen List` (NOT "Screen Route Table") — `validate_feature_spec.py` REQUIRED_H2_SCR enforces `## Screen List` → `## User Journey`. Routes live in `route-list.md`, not this table.
