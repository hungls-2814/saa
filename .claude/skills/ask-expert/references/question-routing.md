...
| **Impact analysis** | "if I change X what's affected", "blast radius", "what depends on X", "ripple" | feature `technical-spec.md` + `docs/generated/entities.md` + `docs/generated/route-list.md` + `docs/generated/behavior-logic.md` + `docs/generated/api-map.md` | grep code refs via `tkm:scan-codebase` | impact three-tier breakdown | Risk Analyst |
| **Improvement opportunities** | "improvement proposal", "upsell", "what can we sell/optimize", "tech debt to pitch" | `plans/improvement-proposal/**` proposals | `feature-list.md`, `docs/project-roadmap.md` | opportunity list w/ rationale | Tech Strategist + Product |
...
- Improvement proposal: *improvement proposal / upsell / opportunity / improvement / proposal / optimize / sell / pitch / monetize / tech debt*
...
| Architecture | scan-codebase for dirs/entrypoints → infer + describe diagram, flag INFERRED + advise `/tkm:rebuild-spec` |
| Feature detail | grep the named symbol/route in code → summarize behavior + advise `/tkm:rebuild-spec --features F###` |
| Impact analysis | grep references of the symbol across code → three-tier from usages + advise `/tkm:rebuild-spec` |
| Improvement proposal | no `plans/improvement-proposal/` → quick heuristic scan (tests, deps, TODOs) + advise `/tkm:propose-improvements` |
...
`--level high` / `--level max` raise the level: `--level high` always reaches source (noting code refs + verify);
`--level max` forces a full source scan plus multi-subagent fan-out to cross-verify every claim. `--level low`
