...
1. `grep` the `target` symbol/route across source (use `tkm:scan-codebase` only when you need broad
   discovery, not for a known symbol).
2. Open only the **enclosing block** (function / class / section) of the top matches — not whole
   files. This counters grep's context-loss (a bare matched line omits its surrounding logic).
3. Caps: ≤ 6 matched files, ≤ ~24 KB read in the escalation step.
4. Cite the code files actually read.
