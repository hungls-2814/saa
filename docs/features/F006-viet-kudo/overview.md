...
- NEW `delete own kudos` policy (`sender_id = auth.uid()`) backs `createKudoAction`'s
  compensating rollback: if the hashtag/image inserts fail after the kudos row commits, the
  action deletes the orphan row (junction/child rows cascade) so the ≥1-hashtag invariant holds.
- GRANT insert on `kudos`, `kudos_hashtags`, `kudos_images`, `hashtags` to `authenticated`;
  GRANT delete on `kudos` to `authenticated` (own-row only, enforced by the policy above).
- Migration: `supabase/migrations/20260708150000_kudos_compose.sql`. On hosted Supabase the
  `storage.objects` policies in that file may need applying via the SQL editor (ownership).
