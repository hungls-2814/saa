...
- [x] Hero banner shows art + `ROOT FURTHER` + `Hệ thống giải thưởng SAA 2025`.
- [x] Sidebar lists 6 awards in order; active = gold+underline; click scroll-spies.
- [x] 6 detail sections render with correct title/desc/quantity/prize + `#slug` anchors.
- [x] Deep-link `#<slug>` from homepage scrolls to the section.
- [x] Homepage award cards / nav / footer now point to `/he-thong-giai(#slug)`.
- [x] VN/EN localized; layout matches the design.

## Key files (shipped)
- Route: `app/he-thong-giai/page.tsx` — server component; `getUser()` → `redirect("/login")` when unauthenticated.
- Components: `app/he-thong-giai/components/{awards-hero,awards-sidebar,award-detail-section,award-icons,use-active-section}.{ts,tsx}`
- Data: `app/he-thong-giai/data/awards-detail-data.ts`
- Guard: `proxy.ts` — `PROTECTED_PATHS = ["/he-thong-giai"]`
- Homepage link updates: `app/(home)/components/{award-card,site-header,site-footer,hero-section}.tsx` now point at `/he-thong-giai(#slug)`; `SiteHeader` gained an `active` prop (`active="awards"` on this page)
