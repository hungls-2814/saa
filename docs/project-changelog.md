...
### Changed
- **Favicon / tab icon** — regenerated `app/favicon.ico` (multi-size 16/32px PNG-in-ICO) and
  added `app/icon.png` (256×256) for modern browsers, both cropped from the red Sun* "S" mark
  in `public/login/logo.png`. The wordmark text is dropped so the glyph stays legible at tab
  size. Picked up automatically via Next.js App Router file-convention icons — no
  `metadata.icons` boilerplate needed.

### Fixed
- **ESLint config** — added `.vercel/**` to `globalIgnores` so autogenerated (gitignored)
  Vercel build output is no longer linted, clearing false-positive errors from `npm run lint`.
