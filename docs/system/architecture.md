...
## Lint
`eslint.config.mjs` overrides `eslint-config-next`'s default ignores to also exclude `.vercel/**`
(autogenerated Vercel build output, gitignored), `.claude/**`, and `plans/**` (agent-kit tooling
and workspace artifacts) — none of these are application source.
