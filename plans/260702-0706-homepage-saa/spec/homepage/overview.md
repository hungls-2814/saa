...
## Acceptance criteria
- [ ] Homepage renders at `/` for anonymous and authenticated users (public content).
- [ ] Authenticated header shows notification bell + account menu; anonymous does not.
- [ ] Countdown reads env datetime, decrements, 0-pads, and zeroes out + hides "Coming
      soon" at/after target; invalid datetime does not crash.
- [ ] Awards grid: 3 cols desktop, 2 cols tablet/mobile; cards link to `#<slug>`.
- [ ] All nav/CTA/footer/card links point to correct hrefs; logo scrolls to top.
- [ ] VN/EN switch updates all copy and persists via `NEXT_LOCALE` cookie.
- [ ] Layout matches the MoMorph design at desktop and narrow widths.
