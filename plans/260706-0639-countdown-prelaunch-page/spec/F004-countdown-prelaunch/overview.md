...
## Behaviour & logic
- **Datetime source:** `NEXT_PUBLIC_EVENT_DATETIME` (ISO-8601, +07:00) with
  `DEFAULT_EVENT_DATETIME` fallback — reused from `lib/event/countdown.ts`.
...
## Reuse (DRY)
- `lib/event/countdown.ts` — `getCountdown`, `parseEventDate`, `DEFAULT_EVENT_DATETIME`.
- LED `CountdownUnit` visual from `app/(home)/components/countdown.tsx` (shared).
